...
def get_monthly_summary(year, month):
    ...
    overall_income = db.session.query(func.sum(Transaction.amount)).filter(
        Transaction.user_id == current_user.id,
        Transaction.type == "Receita"
    ).scalar() or 0.00
    overall_expenses = db.session.query(func.sum(Transaction.amount)).filter(
        Transaction.user_id == current_user.id,
        Transaction.type == "Despesa"
    ).scalar() or 0.00
...
def get_pending_summary():
    ...
    pending_expenses_query = db.session.query(func.sum(Transaction.amount), func.count(Transaction.id)).filter(
        Transaction.user_id == current_user.id,
        Transaction.type == "Despesa",
    ...
    pending_income_query = db.session.query(func.sum(Transaction.amount), func.count(Transaction.id)).filter(
        Transaction.user_id == current_user.id,
        Transaction.type == "Receita",
