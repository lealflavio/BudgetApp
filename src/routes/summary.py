...
def calculate_balance(user_id: int, account_id: int | None = None, end_date: date | None = None) -> Decimal:
    ...
    # Separate sums for income and expense
    income_query = query.filter(Transaction.type == TRANSACTION_TYPE_RECEITA)
    expense_query = query.filter(Transaction.type == TRANSACTION_TYPE_DESPESA)
...
@login_required
def api_get_balance():
    """API: Get the current overall balance for the user (JSON)."""
    try:
        balance = calculate_balance(current_user.id)
        return jsonify({"balance": str(balance)}) # Convert Decimal to string
...
def api_get_pending_summary():
    ...
    try:
        ...
        total_pending_expenses: Decimal | None = pending_expenses_query.scalar()
        ...
        total_pending_income: Decimal | None = pending_income_query.scalar()
