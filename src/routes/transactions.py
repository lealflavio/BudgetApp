...
# These endpoints provide full CRUD (Create, Read, Update, Delete) functionality
# for transactions via a consistent JSON interface.

@transactions_bp.route("/api", methods=["GET"])
@login_required
def api_list_transactions():
    """API: List transactions (Read - Multiple) for the current user (JSON). Supports filtering."""
    try:
...
def api_get_transaction(transaction_id: int):
    """API: Get a single transaction (Read - Single) by ID (JSON)."""
...
def api_add_transaction():
    """API: Add a new transaction (Create) (JSON)."""
...
def api_edit_transaction(transaction_id: int):
    """API: Edit an existing transaction (Update) (JSON). Supports partial updates (PATCH)."""
    transaction: Transaction | None = Transaction.query.filter_by(id=transaction_id, user_id=current_user.id).first()
    if transaction is None:
        return jsonify({"error": "Transação não encontrada ou não autorizada"}), 404
...
def api_delete_transaction(transaction_id: int):
    """API: Delete a transaction (Delete) (JSON)."""
    transaction: Transaction | None = Transaction.query.filter_by(id=transaction_id, user_id=current_user.id).first()
    if transaction is None:
        return jsonify({"error": "Transação não encontrada ou não autorizada"}), 404
