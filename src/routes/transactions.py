...
class TransactionForm(FlaskForm):
    ...
    date = DateField(
        "Data", format="%Y-%m-%d", default=date.today, validators=[DataRequired()]
    )
    ...
    category = QuerySelectField(
        "Categoria",
        query_factory=lambda: get_user_categories("Despesa"),
        get_label="name",
        allow_blank=False,
        validators=[DataRequired()],
    )  # Default to Despesa
...
def add_transaction():
    ...
    form.category.query_factory = lambda: get_user_categories(
        form.type.data or "Despesa"
    )
    ...
    # Temporary response
    return jsonify(
        {"message": "GET request to add_transaction. Use POST to submit form."}
    )
...
def edit_transaction(transaction_id):
    ...
    # Dynamically set category choices based on initial/POSTed type
    form.category.query_factory = lambda: get_user_categories(
        form.type.data or transaction.type
    )
    ...
    if form.validate_on_submit():
        ...
        transaction.is_received = (
            form.is_received.data if form.type.data == "Receita" else True
        )
        db.session.commit()
    ...
    return jsonify(
        {
            "message": f"GET request to edit_transaction {transaction_id}. Use POST to submit form."
        }
    )


@transactions_bp.route(
    "/transactions/delete/<int:transaction_id>", methods=["POST"]
