...
class CategoryForm(FlaskForm):
    name = StringField(
        "Nome da Categoria", validators=[DataRequired(), Length(max=100)]
    )
    type = SelectField(
        "Tipo",
...
def add_category():
    ...
    if form.validate_on_submit():
        ...
        existing_category = Category.query.filter_by(
            user_id=current_user.id, name=form.name.data, type=form.type.data
        ).first()
...
def edit_category(category_id):
    ...
    return jsonify(
        {
            "message": f"GET request to edit_category {category_id}. Use POST to submit form."
        }
    )


@categories_bp.route(
    "/categories/delete/<int:category_id>", methods=["POST"]
